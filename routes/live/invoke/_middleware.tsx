...
import { ISABELA_DIAS_CLIENT_COOKIE, ISABELA_DIAS_SESSION_COOKIE, ISABELA_DIAS_WISHLIST_IDS, } from "$store/packs/constants.ts";
...
const setCookies = (res: Response, tokens: Tokens[]) => {
    tokens.forEach(({ tokenName, tokenValue }) => {
        res.headers.append("Set-Cookie", `${tokenName}=${tokenValue}; Expires=${new Date().setFullYear(new Date().getFullYear() + 1)}; Path=/; Secure; HttpOnly`);
    });
};
export const handler = async (req: Request, ctx: MiddlewareHandlerContext<DecoState<Record<string | number | symbol, never>, Record<string | number | symbol, never>, 
...
Manifest>>) => {
    ...
    const customerToken = await ctx.state.invoke("../deco-sites/otica-isabela/loaders/store/session.ts", { sessionToken: cookies[ISABELA_DIAS_SESSION_COOKIE] });
    setCookies(res, [
        {

...
function ProductInfo({ page, promotions, labels, stepLabels, customer }: Props) {
    ...
    const experimenterImage = additionalProperty?.find((prop) => prop.propertyID === "experimentador")?.value;
    const colorsList = additionalProperty?.filter((prop) => prop.propertyID === "color");
    ...
    const promotionFlag = additionalProperty?.find((prop) => prop.propertyID === "flag")?.value;
    const promotion = promotions?.find((current) => current.label === promotionFlag);
    const currentCategory = breadcrumbList?.itemListElement[0].name;
    const rating = additionalProperty?.find((prop) => prop.propertyID === "rating")?.value;
    const isAllowedToAddLens = additionalProperty?.find((prop) => prop.propertyID === "isAllowedToAddLens");
    const isLensWithoutPrescription = additionalProperty?.find((prop) => prop.propertyID === "isLensWithoutPrescription")?.value;
    const lensDescription = additionalProperty?.find((prop) => prop.propertyID === "lensDescription")?.value;
    ...
    return (<>
      ...
      <div class="mb-4 flex flex-start">
        <div class="flex flex-col">
          ...
          {lensDescription && (<span class="font-roboto font-medium text-sm">
              {lensDescription}
            </span>)}
        </div>
        <div class="ml-2">
          <WishlistButton variant="icon" productID={productID} customer={customer}/>
        </div>
      </div>
      ...
      <div class="flex items-normal justify-between">
        <div class="flex flex-col gap-2">
          {listPrice !== price && (<p class="line-through font-semibold text-sm  text-red-500 lg:text-base">
        ...
        <div class="flex flex-col items-end justify-between ml-2 gap-2">
          {!!ratingValue && (<a href="#product-review" aria-label="Veja as avaliações!">
              <Ratings ratingValue={ratingValue}/>
            </a>)}

          {!!colorsList?.length && (<div class="flex gap-2 items-center">
              <p class="text-base-300 font-bold">
                {colorsName?.join(" / ").toUpperCase()}
              </p>
              <span class="ml-2 block bg-red-500 w-[25px] h-[30px] rounded-xl border-2 border-gray-300" style={{
      ...
      {!isLentes && experimenterImage
            ? (<div class="mt-4">
            <ToExperimentButton image={experimenterImage!} variant="filled" size="small"/>
          </div>)
            : null}

      {/* Choose Lens */}
      {stepLabel && isAllowedToAddLens && (<div class="mt-[11px] w-full">
          <ChooseLensButton {...addToCard} text={stepLabel} chooseLensUrl={chooseLensUrl}/>
        </div>)}

      {/* Add To Cart & Whislist */}
      {!isLentes && (<div onClick={() => {
          ...
          <AddToCartButton {...addToCard} label={labels?.[currentCategory!.toLowerCase()]}/>
        </div>)}

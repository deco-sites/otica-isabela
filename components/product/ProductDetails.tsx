...
import { type SectionProps, redirect, type LoaderReturnType } from "@deco/deco";
type ButtonLabel = {
    category: string;
    label: string;
};
...
function ProductDetails({ page, measurementsImage, promotions, buttonByCategory, stepButtonByCategory, customer, mobileOptions, }: SectionProps<typeof loader>) {
    const { product } = page || {};
    ...
    return (<>
      <div class="lg:bg-gray-scale-100">
        {/* Stopwatch */}
        {priceValidUntil && (<BestOffersHeader priceValidUntil={priceValidUntil!} page="details"/>)}
        <div class="container py-[20px]">
          {page
            ? (<Details page={page} promotions={promotions} buttonByCategory={buttonByCategory} stepButtonByCategory={stepButtonByCategory} customer={customer} mobileOptions={mobileOptions}/>)
            : <NotFound />}
        </div>
      </div>
...
export function loader(props: Props, req: Request, ctx: AppContext) {
    ...
    const currentIds: string[] | undefined = cookies?.[visitedProductsCookie]?.split(":") ?? [];

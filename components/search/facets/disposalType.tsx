...
interface DisposalTypeFacetsProps {
...
export default function DisposalTypeFacets({
  facets,
  handleFilterChange,
  ...
  return (
    <details class="group">
      ...
      <ul class="flex flex-wrap gap-3.5 flex-col mb-6 pt-2.5">
        {Object.entries(disposalTypeFacets).slice(0, 10).map((
          [disposal, count],
        ) => {
          const uniqueInputId = `${componentId}-disposal-${disposal}`;
          ...
          return (
            ...
            >
              <input
                id={uniqueInputId}
                type="checkbox"
                checked={filters.isFilterActive("Tipo de Descarte", disposal, "in")}
                class="hidden"
                onChange={(e) =>
                  handleFilterChange(
                    "Tipo de Descarte",

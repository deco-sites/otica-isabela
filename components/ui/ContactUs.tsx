...
export default function ({ title, cards, bottomText, isMobile, }: ReturnType<typeof loader>) {
    return (<div class="bg-[#f3f3f3] flex flex-col pt-5 text-black">
      ...
      <div class="flex flex-col min-[768px]:flex-row gap-x-10 gap-y-2.5 mb-10 max-w-[1140px] mx-auto max-[768px]:w-[95%]">
        {cards.map((card) => (<a href={card.link} class="flex min-[768px]:flex-col items-center gap-2.5 py-4 min-[768px]:pt-10 min-[768px]:pb-8 px-5 bg-white hover:shadow-[0_0_18px_rgba(0,0,0,.12)] min-[768px]:min-h-[260px] w-full min-[768px]:w-1/3 rounded-[10px] group">
            <Image src={card.icon} alt={card.alt} width={isMobile ? 26 : 36} height={isMobile ? 26 : 36}/>
            <strong class="font-medium text-xl min-[768px]:text-2xl group-hover:text-[#d39d4e] min-[768px]:ml-1">
              {card.title}
            </strong>
            <div class={" text-[#646464] text-lg leading-[1.3]" +
                " [&_strong]:text-black [&_strong]:text-base text-center max-[768px]:ml-auto"}>
              <div dangerouslySetInnerHTML={{
                __html: isMobile ? card.mobileText ?? "" : card.text,
            }}/>
            </div>
            {!isMobile && (<span class="flex justify-center items-center bg-[#d39d4e] hover:bg-[#dcb171] rounded-md transition-colors text-center select-none outline-0 cursor-pointer whitespace-nowrap w-full min-[768px]:w-[200px] h-10 px-5 mt-[12px] max-w-[300px] text-white">
                {card.buttonText}
              </span>)}
          </a>))}
      </div>

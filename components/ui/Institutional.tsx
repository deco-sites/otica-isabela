import SideMenuNavigation, { Navigation, } from "deco-sites/otica-isabela/components/ui/SideMenuNavigation/SideMenuNavigation.tsx";
...
function findNavigationAndParents(navigations: Navigation[], pathname: string): null | {
...
export default function Institutional({ header = "Instutional", breadcrumb = { href: "/", label: "Institutional" }, navigations = [
    { content: [], href: "/", label: "Home" },
    { content: [], href: "/", label: "Home" },
    { content: [], href: "/", label: "Home" },
], url = "/", style = { activeColor: "#d6a35a", color: "#000000" }, isMobile = false, }: SectionProps<typeof loader>) {
      ...
      <div class="bg-[#efefef] lg:bg-transparent py-5">
        ...
        {isMobile && (<>
            <div class="mt-4 grid grid-rows-[auto_0fr] [&:has(>#menu:checked)]:grid-rows-[auto_1fr] transition-[grid-template-rows] duration-200 overflow-hidden relative bg-white">
              ...
              <label htmlFor="menu" class="cursor-pointer col-start-1 row-start-1 z-10"/>
              <div class="container w-full h-fit min-h-14 col-start-1 row-start-1 cursor-pointer flex gap-3 items-center p-4 sm:p-0 justify-between peer-checked:[--icon-rotate:90deg]">
                ...
                <Icon id="ChevronRight" size={24} strokeWidth={3} class="transition-all" style={{ rotate: "var(--icon-rotate, 0deg)" }}/>
              </div>
              <div style={{ "--color-active": activeColor, "--color": color }} class="invisible col-start-1 row-start-2 min-h-0 peer-checked:visible peer-checked:min-h-fit transition-all container px-4 peer-checked:pb-4">
                <SideMenuNavigation activeNavigations={hrefs} navigation={navigations}/>
              </div>
            </div>
          </>)}
      </div>
      <div class="flex container gap-10 py-10 px-3 md:px-0" style={{ "--color-active": activeColor, "--color": color }}>
        {!isMobile && (<div class="max-w-48 w-full h-full">
            <h2 class="text-lg font-semibold mb-2">{header}</h2>
            <hr />
            <SideMenuNavigation activeNavigations={hrefs} navigation={navigations}/>
          </div>)}
        <div class="w-full">
          {navigation?.content.map((section) => (<section.Component {...section.props}/>))}
        </div>
      </div>
    </div>);
}

...
export default function SideMenuNavigation({ navigation, activeNavigations, level = 0, }: Props) {
    return (<Wrapper level={level}>
      <ul style={{ paddingLeft: `${20 * level}px` }}>
        {navigation.map((nav) => {
            ...
            return (<li class="mt-2">
              <a class={"hover:text-[var(--color-active)] hover:underline" +
                    (isActive
                        ? " text-[var(--color-active)]"
                        : " text-[var(--color)]")} href={nav.href}>
                {nav.label}
              </a>
              {nav.navigations && hasActiveChild && (<SideMenuNavigation navigation={nav.navigations} activeNavigations={activeNavigations} level={level + 1}/>)}
            </li>);
        })}
      </ul>
    </Wrapper>);
